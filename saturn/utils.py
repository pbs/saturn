...
def get_rules_by_prefix(prefix):
    ...
    for rule in rules:
        ...
        results.append({
            ...
            'target_command': ' '.join(json.loads(targets[0]['Input'])['containerOverrides'][0]['command'])
        })
    return results
...
def get_log_for_run(log_group_name, log_stream_name):
    ...
    while True:
        try:
            events = logs.get_log_events(logGroupName=log_group_name,
                                         logStreamName=log_stream_name, **extra)
        except ClientError:
